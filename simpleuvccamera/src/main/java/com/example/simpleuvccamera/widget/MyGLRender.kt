    ...
    fun feedData(buf:ByteBuffer){
        synchronized(this) {
            if (hasVisibility) {
                ...
                v.clear()
                buf.clear()
                buf.limit(mVideoWidth*mVideoHeight)
                y.put(buf)
                buf.limit(mVideoWidth*mVideoHeight + mVideoWidth * mVideoHeight / 4)
                u.put(buf)
                buf.limit(mVideoWidth*mVideoHeight + mVideoWidth * mVideoHeight / 2)
                v.put(buf)
            }
        }
    }
